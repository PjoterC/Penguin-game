...
void movement(int* pBoard, int rows, int columns, int penguinNum, int currPlayer, int* pPlayerScore){
    ...
    if(checkP == true){
        ...
        while(k == 1){
            ...
            while((scanf("%d", &dir))<1){
                scanf("%*[^\n]");
                printf("WRONG INPUT! Direction has to be a positive integer\n");
                printf("In what direction would you like to move?\n");
                printf("1 - up\n");
                printf("2 - down\n");
                printf("3 - left\n");
        ...
        switch (dir){
                ...
                if(*(pCurrPos-columns) == 1 || *(pCurrPos-columns) == 2 || *(pCurrPos-columns) == 3){
                    ...
                    *(pCurrPos-columns) = playerPenguin;
                    k = 0;
                } else printf("You cannot move in this direction\n");
                break;
                case 2:
                if(*(pCurrPos+columns) == 1 || *(pCurrPos+columns) == 2 || *(pCurrPos+columns) == 3){
                *(pPlayerScore + currPlayer - 1) += *(pCurrPos+columns);
